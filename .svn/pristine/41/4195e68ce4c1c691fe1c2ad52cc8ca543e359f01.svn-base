...
{
    class sql_supplier:IDAL.supplierDal
    {
        ...
        public int getMax()
        {
            object x = sql.ExecuteSc("select top 1 id from tab_suppliers supplier by id desc");
            return Convert.ToInt32(x) + 1;
        }
        public Model.tab_suppliers getsuppliers(Model.tab_suppliers suppliers1)
        {
            StringBuilder strsql = new StringBuilder();
            strsql.Append("select * from tab_suppliers where ");
            strsql.AppendFormat("id='{0}'", suppliers1.id);
            DataTable dt = sql.ExecuteDataSet(strsql.ToString()).Tables[0];
            if (dt.Rows.Count < 1) return null;

            Model.tab_suppliers suppliers = new Model.tab_suppliers();
        ...
        {
            return sql.ExecuteDataSet("select * from tab_suppliers").Tables[0];
        }
        ...
        public int Add(Model.tab_suppliers suppliers)
        {
            StringBuilder strsql = new StringBuilder();
            strsql.Append("insert into tab_suppliers values (");
        ...
        public int update(Model.tab_suppliers suppliers)
        {
            StringBuilder strsql = new StringBuilder();
            strsql.Append("update tab_suppliers set ");
